...
export const Register = () => {
  ...
  const mutation = useMutation({
    ...
    onError: (error) => {
      setError('login', { message: error.message || 'Регистрация не удалась' });
    },
  });
  ...
  const onSubmit = (data) => {
    ...
    mutation.mutate({ login: data.login, password: data.password, name: data.name });
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center">
      ...
      >
        ...
        {errors.confirmPassword && <p className="text-red-500 text-sm text-center">{errors.confirmPassword.message}</p>}
        ...
        <div className="text-center text-sm text-gray-500">
          Уже есть аккаунт?{' '}
          <NavLink to="/login" className="text-orange-500 hover:underline transition-colors duration-200">
            Войти
          </NavLink>
        </div>
      </form>
